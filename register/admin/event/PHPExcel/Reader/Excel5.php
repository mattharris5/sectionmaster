...
class PHPExcel_Reader_Excel5 implements PHPExcel_Reader_IReader
{
		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					$ranges = explode(',', $definedName['formula']); // gdpFIXME: what if sheetname contains comma?
	...
	private function _readLabel()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell value
			// gdptodo: what if string is very long? continue record
			if ($this->_version == self::XLS_BIFF8) {
	...
	{
		// gdptodo: if cell range is just a single cell, should this funciton
		...
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					...
					// gdptodo: check if we have identified the whole set of special characters
				...
				default:
					// gdpTODO: external sheet support

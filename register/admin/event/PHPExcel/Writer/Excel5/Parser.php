...
class PHPExcel_Writer_Excel5_Parser
{
	...
	function _convertRange2d($range, $class=0)
	{
		...
		// gdpTODO: possible class value 0,1,2 check Formula.pm
	...
	function _rangeToPackedRange($range)
	{
		...
		$col1 = 0;
		$col2 = 16383; // gdpFIXME: maximum possible value for Excel 5 (change this!!!)

		// gdpFIXME: this changes for BIFF8
		...
		if ($this->_BIFF_version == 0x0500) {
			$row1    |= $row1_rel << 14; // gdpFIXME: probably a bug
			$row2    |= $row2_rel << 15;
			$col1     = pack('C', $col1);
			$col2     = pack('C', $col2);
		} elseif ($this->_BIFF_version == 0x0600) {

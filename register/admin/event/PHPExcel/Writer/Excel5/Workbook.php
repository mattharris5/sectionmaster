...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
								...
								)
	{
		...
		$this->_biffsize         = 0;
		$this->_palette          = array();
		$this->_codepage         = 0x04E4; // gdpFIXME: should change for BIFF8
	...
	public function writeWorkbook($pWorksheetSizes = null)
	{
		...
		if ($this->_BIFF_version == 0x0600) {
			...
			/* gdpTODO: store external SUPBOOK records and XCT and CRN records
	...
	private function _writeExternsheetBiff8()
	{
		...
		$supbook_index = 0;           // gdpFIXME: only using internal SUPBOOK record
		$header           = pack("vv",  $record, $length);

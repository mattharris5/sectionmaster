...
/*	SM-Online Registration System                      www.sectionmaster.org
	...
	if (postValidate()) {
		...
		// confirm inventory
		$sql = "SELECT ordered_item_id, products.item, quantity, 
				...
				HAVING inventory < ordered_items.quantity";
		$res = $db->query($sql);
		...
		if ($_REQUEST['payment_method'] == 'cc' && !$ERROR['inventory']) {
			...
			if ($order_complete != '1') {		// note: this used to be "if ($member->registration_complete != '1')", but that made no sense
				...
				$url = "https://www.viaklix.com/process.asp?" 
					...
					. "&ssl_exp_date=" . urlencode($_REQUEST['exp_month'] . $_REQUEST['exp_year'])
					. "&ssl_country=" . urlencode(substr($_REQUEST['ssl_card_number'], 0, 2)) 
					. "****" . urlencode(substr($_REQUEST['ssl_card_number'], -4))
					. "&ssl_merchant_id=MERCHANT_ID"
					. "&ssl_user_id=MERCHANT_USER_ID"
					. "&ssl_pin=MERCHANT_PIN"
					. "&ssl_show_form=false"
					. "&ssl_invoice_number=" . urlencode($EVENT['section_name'] . "/"
						. str_pad($EVENT['id'], 3, '0', STR_PAD_LEFT) . "-" 
						. str_pad($_SESSION['order_id'], 7, '0', STR_PAD_LEFT))
					. "&ssl_cvv2=PRESENT"
					. "&ssl_salestax=0.00"
					. "&ssl_description="
						. urlencode($EVENT['section_name'] . " " 
						. $EVENT['formal_event_name']  . " Order No. " 
						. str_pad($_SESSION['order_id'], 7, '0', STR_PAD_LEFT))
					. "&ssl_customer_code=1"
					. "&ssl_result_format=ASCII";
				
				// Go retrieve the results from the viaKlix site
				$lines = file($url, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
				$cc_result = array();

				// Fill up the $cc_result array with the return values from viaKlix
	...
	// Check response from viaKLIX
	if ($cc_result['ssl_result'] == '0') {

		// ok - proceed to final	
		require "includes/complete_order.php";

	} elseif ($cc_result['ssl_result'] != '0' && $cc_result['ssl_result']) {  // declined
		
		$ERROR['cc'] = "<div class=\"error_summary\"><h3>Error Processing Card</h3>
						There was a problem processing your credit card.";
		
		if ($cc_result['ssl_cvv2_response'] == 'N')			
			$ERROR['cc'] .= " The Card Security Code is incorrect.";
		
		$ERROR['cc'] .= "  Please try again or contact customer support for assistance.</div>";
	...
	if ($_REQUEST['coupon_code'] != '')
		$sql = "SELECT * FROM products 
				WHERE coupon = '1' AND UPPER(product_code) = '" . strtoupper($_REQUEST['coupon_code']) . "'";
		...
		$sql = "SELECT * FROM products WHERE coupon = '1' AND auto_add_coupon = '1' AND inventory > 0";
